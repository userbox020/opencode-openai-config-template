...
- When the user explicitly requests planning only, do not implement or dispatch write-capable agents. Create the plan and stop; recommend OpenCode's native Plan agent when a permission-enforced planning boundary is important.
...
- Use `@oracle` for architecture, persistent debugging, and high-risk reasoning or review.
